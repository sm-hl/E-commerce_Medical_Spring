...
<link rel="stylesheet" href="/headers.css" media="screen">
  ...
  <body onload="populateTableList()" class="u-body u-overlap u-overlap-contrast u-stick-footer">
          ...
          <main>
            ...
            <c:import url="/inc/panel_stocks.jsp"/>
            ...
            <div class="text-center my-5">
                <span class="fs-3 text-gray">Ajouter des nouveaux produits au stock, ou l'accumuler s'il existe déjà!</span>
            </div>
            <form method="POST" action="/Admin/ajouterStocks" enctype="multipart/form-data">
                <div class="table-responsive my-5">
                    <table class="table table-primary table-striped">
                        <thead>
                            <tr>
                                <th scope="col"><label for="produitInput" class="form-label">Produit</label></th>
                                <th scope="col"><label for="nbStockInput" class="form-label">Nombre à ajouter</label></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>
                                    <select id="produitInput" class="form-select" aria-label="Catégorie" name="idProduit">
                                        <c:forEach items="${ produits }" var="produit">
                                            <option value="${produit.getId()}">${produit.getNomProduit()}</option>
                                        </c:forEach>
                                    </select>
                                </td>
                                <td><input type="number" class="form-control form-control-sm" id="nbStockInput" name="nbStock" required></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
